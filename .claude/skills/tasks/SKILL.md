...
2. **ビルド順で分解**（Clean Architecture の内側から）: domain（エンティティ／値オブジェクト）→ application（ユースケース＋リポジトリ抽象）→ infrastructure（ORMモデル・リポジトリ実装）→ adapters（schemas・api）→ composition root → frontend。
3. **各タスクに、検証すべき既存シナリオを紐付ける**（`@R-x` で参照）。「このタスクが green にすべきシナリオ」を明記する。
4. **TDD を前提に順序付け**: 各実装タスクは「対象シナリオを pytest-bdd 化して red → 最小実装で green → refactor」を内包する。依存（内側の層）が先。
5. 各タスクは小さく検証可能に。完了条件（どのテスト／チェックが通れば終わりか）を書く。
6. 人間レビューに出す。承認まで implement へ進まない。
