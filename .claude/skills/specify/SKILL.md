...
argument-hint: [feature-folder 例: 0001-shooting-session]
...
| 段 | もの | 置き場 | 個数 |
| --- | --- | --- | --- |
| 機能 | ユーザーが意味のある1タスクを完了できる縦切り（例: シューティングセッション） | `specs/NNNN-name/`（フォルダ） | 1 |
| 要件 | 機能を構成する個々のルール（EARS） | requirements.md（1ファイル）内に複数、ID `R-1`… | 数件〜十数件 |
| シナリオ | 各要件の具体例（Gherkin） | acceptance.feature（1ファイル）内に複数、`@R-1` タグで紐付け | 各要件に複数 |

**機能フォルダの粒度の目安**: そこから EARS 要件が数件〜十数件出るくらい。要件が1件しか出ないなら小さすぎ（それは機能ではなく要件）。無関係な複数ゴールが混ざるなら大きすぎ（分割する）。例:「ヒットの記録」は機能ではなく shooting-session 内の要件 `R-1`。
