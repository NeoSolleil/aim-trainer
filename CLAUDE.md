...
- .claude/agents/ … 役割別サブエージェント。**ワーカー役**（product-analyst・quality-analyst・solution-analyst・scenario-author・backend-engineer・frontend-engineer）が各段階で Skill を呼んで実作業し、**レビュー役**（spec-compliance・test-coverage・code-reviewer）が独立コンテキストで検証する。各エージェントは独立コンテキストで動く。
- .claude/settings.json … Claude Code の PreToolUse フック登録。アーキ違反（domain/application への FW import）・直接SQL・pre-commit 迂回（--no-verify）を編集／実行時に決定論的にブロックする。テスト実行や lint の強制はコミット時の pre-commit と CI（GitHub Actions）が担う。
