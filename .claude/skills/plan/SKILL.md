...
- `backend-architecture` … Clean Architecture・Pydantic v2・依存性逆転。
- `frontend-architecture` … CA＋Atomic Design・Smart/Dumb・SWR（UI を伴う場合）。
- `design` … Tailwind デザイントークン（画面設計に触れる場合）。
...
3. **レイヤ配置（Clean Architecture）**: 何を domain / application（ユースケース＋リポジトリ抽象）/ adapters（api・schemas）/ infrastructure（ORMモデル・リポジトリ実装）に置くかを決める。domain は純粋、ORM とドメインエンティティは分離。
4. **API 契約**: エンドポイント・メソッド・リクエスト/レスポンスの Pydantic DTO・ステータスコード・エラー形式。acceptance シナリオと対応づける。
5. **データモデル**: テーブル（gun / score …）・カラム・型・主キー/外部キー・制約・インデックス。SQLAlchemy マッピング方針（infrastructure に ORM、domain と相互変換）。将来 PostgreSQL 移行可能に保つ。
6. **トレーサビリティ**: 各 R-x → 設計要素（ユースケース / エンドポイント / テーブル）の対応表。
7. 人間レビューに出す。承認まで tasks へ進まない。
...
- データモデル（テーブル定義・FK・制約）
- **フロント設計（UI を伴う場合）**: 画面・コンポーネント構成、Canvas 描画方針、状態とデータ取得、`data-testid` 計画
- 要件↔設計のトレーサビリティ表
