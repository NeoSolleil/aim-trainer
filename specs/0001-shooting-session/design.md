...
この分担は requirements.md の Rule 11（設計の前提）と承認済み決定に従う。@e2e シナリオ（20 件）はフロント、@backend シナリオ（7 件）はバックエンド（domain＋永続化）で検証される（タグが責務分担と一致している）。
...
進行中セッションの状態・判定・計測・タイマー・サマリ・「もう一度」・計上規則をすべて担う（@e2e 20 シナリオ）。React + TypeScript strict（`any` 禁止）。Atomic Design ＋ Smart/Dumb 分離。
...
**網羅集計**: R-1〜R-22 の **22 件すべて**を設計要素に対応づけた。内訳 — フロントのみ: R-1, R-3, R-4, R-5, R-6, R-7, R-8, R-9, R-10, R-17, R-20, R-21, R-22（13 件）／バックエンドのみ: R-11, R-12, R-13, R-14, R-15（5 件）／フロント＋バック両面: R-2（記録=F／非負検証=B）, R-16（通知=F／非2xx=B）, R-18（表示=F／算出・nullable=B）, R-19（表示=F／算出・nullable=B）（4 件）。これは acceptance.feature のタグ分布（@e2e 20・@backend 7）と整合する。
